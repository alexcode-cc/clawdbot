...
```json5
{
  session: {
    maintenance: {
      mode: "warn",         // "enforce" 或 "warn"（預設 "warn"）
      pruneAfter: "30d",    // 自動清理超過 30 天的條目
      maxEntries: 500,      // 限制會話條目數量
      rotateBytes: 10485760 // 會話檔案輪轉（10MB）
    }
  }
}
```
...
{
  cron: {
    sessionRetention: "24h"   // 預設 24 小時，設為 false 停用清理
  }
}
```

### models.providers - 新增提供者
