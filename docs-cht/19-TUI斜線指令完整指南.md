...
## TUI 指令改善（2026.4.13–2026.4.14）

### /trace 診斷增強

`/trace` 指令診斷功能大幅改善（#66089）：

```
/trace          # 切換 trace 模式
/trace raw      # 顯示原始 trace 資料
/trace active   # 顯示 active memory recall 追蹤
```

**改善**：
- 改善 trace raw diagnostics 輸出格式
- 改善 trace 指令 UX（更清晰的輸出結構）
- Active Memory recall 診斷整合更緊密

### /status 改善（Discord）

在 Discord 中，`/status` 現在正確回傳狀態卡（#54629），而非通用 ack 訊息。

### Telegram Forum Topics

在 Telegram Forum Groups 中，agent context 現在包含人類可讀的 topic 名稱（#65973），可透過 `/session` 和 `/agent` 指令查看當前 session 綁定的 topic 資訊。

---

## TUI 指令改善（2026.4.15）

### TUI Streaming Watchdog

新增客戶端串流 watchdog（#67401）：
- `streaming · Xm Ys` 活動指示器在 30 秒 delta 靜默後自動重置為 `idle`
- 防止 WS 重連、gateway 重啟等情況造成 TUI 永久卡在 `streaming` 狀態
- 觸發時輸出系統日誌行，提示使用者發送新訊息以重新同步
- 可透過 `streamingWatchdogMs` context option 配置（設為 `0` 禁用）

### /skills 快取失效感知

當配置寫入觸及 `skills.*` 時，session skills snapshot 會更新：
- 移除 skills 後 agent 不再繼續呼叫已停用的工具
- `/skills` 指令可查看當前有效的 skill 列表

### Unknown-Tool Guard 回饋

Unknown-tool guard 現在預設啟用，TUI 會顯示：
- 幻覺工具呼叫觸發 guard 時的清晰錯誤訊息
- 告知使用者哪個工具名稱不存在

### CLI Transcript 持久化

CLI-backed turns（如 google-gemini-cli）現在持久化至 session transcript：
- 在 `/session` 指令中可見
- Control UI 中也可查看 CLI 回覆歷史

---

## TUI 指令改善（2026.4.18–2026.4.20）

### Control UI Settings 全面改版（2026.4.18）

Control UI 的 settings 和 slash-command 體驗全面翻新（#67819）：
- 更快速的 presets
- Quick-create flows 讓建立配置更簡便
- 重新設計的 command discovery

### Session Auto-Override 清除（2026.4.20）

`/new` 和 `/reset` 現在清除 auto-sourced overrides（#69419, #69365）：
- 清除 auto-sourced model、provider 和 auth-profile overrides
- 保留明確的使用者選擇
- Channel sessions 不再停留在 runtime fallback 選擇上

### Compaction 通知（2026.4.20）

Context compaction 期間可選收到通知（#67830）：
- opt-in 開始和完成通知
- 適合需要觀察 compaction 行為的使用者

---

## TUI 指令改善（2026.4.22–2026.5.12）

### 2026.5.12（精選）

- **`/plugins` help**：裸 plugin help 保持輕量路徑，不先觸發昂貴的 registry discovery，TUI 內顯示會更快。
- **`/acp`**：ACP turn 若配置了 fallback backend，TUI 的對應說明現在會反映可退路的執行語意。
- **`/status` / `whoami`**：文件與實作都更強調 current-turn / session-history 的一致性，而不是只看單一 runtime snapshot。
...
- **`/think default`**、**`/fast default`**：清除 session override（與 chat 指令一致，#79385）。
- **Control UI／native chat**：thinking 繼承預設與明確 override **分開標示**；compact **context usage** 指示器（#46398 等）。

## TUI 指令改善（2026.4.22–2026.5.7）（歷史滾動）
...
- `/status` 顯示 **`Runner:`**，區分 embedded Pi、CLI、`codex (acp)` 等後端

### Heartbeat／提示詞（2026.4.24）

- **Heartbeat 專用系統片段**不再混入一般對話／非 heartbeat 輪廓（changelog：agents heartbeat prompt isolation）。
...
*本文件基於 OpenClaw `2026.5.12` 版本撰寫。完整英文文件請參考 https://docs.openclaw.ai/tools/slash-commands*
