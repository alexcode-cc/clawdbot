...
## 安裝與設定改善（2026.4.13–2026.4.14）

### Hostinger VPS 安裝

2026.4.14 新增 Hostinger VPS 安裝指南（英文文件：`docs/install/hostinger.md`）。Hostinger 提供平價 VPS 方案，適合個人部署 OpenClaw。

### Docker-out-of-Docker 限制說明

2026.4.14 新增 Docker-out-of-Docker Paradox 文件。若您在 Docker 容器內執行 OpenClaw Gateway，需注意：
- 容器內的 Docker socket 需明確 mount
- 部分工具（exec sandbox）在 DooD 環境中行為可能不同
- 詳見英文文件 `docs/gateway/docker.md`

### Onboarding Probe 改善

- OpenAI 相容端點驗證探測使用 `max_tokens=16`（#66450），修復嚴格端點拒絕 onboarding 的問題
- 適用於 WuKongAI-CMU、自架 OpenAI-compatible 端點等嚴格驗證的服務

### Doctor 安全改善

- `openclaw doctor --repair` 不再將 dotenv secrets 重新嵌入 systemd units（#66249）
- 診斷更安全：保留較新的 inline overrides 優先於舊的 state-dir `.env` 值
...
`openclaw update` 現在更可靠（#66959）：
- 裁剪 npm 升級後過期的打包 `dist` chunks
- 驗證安裝的 package inventory
- 保持降級/升級驗證跨舊版本正常運作

### 全局安裝 Onboarding 修復

修復全局安裝 CLI 在 onboarding 期間頻道選擇崩潰的問題（#66736）。

### Gateway Restart Loop 修復

Linux/systemd 上 plugin auto-enable 不再觸發虛假 SIGUSR1 重啟迴圈（#67557）：
- 修復每次啟動被 chokidar 視為外部變更的問題
- 防止 manifest.db 在重複循環中損壞

### Bundled Plugin Runtime Deps 本地化

捆綁插件 runtime 依賴本地化至擁有的 extensions（#67099）：
- core 不再攜帶 extension-owned runtime 負擔
- 發布的 tarballs 更精簡

### Doctor Config Hash 修復

config 寫入後重讀持久化的 config hash（#66528）：
- 修復配置更新因過期 hash 競態失敗的問題

---

## 安裝與設定改善（2026.4.18–2026.4.20）

### Gateway Stale PID 保護（2026.4.18）

修復 stale-gateway cleanup 終止當前程序父程序的問題（#68517）：
- WeChat 等 plugin sidecars 不再殺死 active gateway
- 防止無限 supervisor 重啟迴圈

### Gateway TUI Startup Retry（2026.4.20）

`openclaw tui` 重連時重試 session history（#69164）：
- 修復 `chat.history unavailable during gateway startup` 錯誤

### Doctor 配對改善（2026.4.20）

`openclaw doctor --fix` 現在顯示（#69210）：
- 待處理的設備配對請求
- Scope-upgrade approval drift
- Stale device-token 修復步驟

### Plugin Bundled Runtime 管理改善（2026.4.18–20）

Bundled plugin runtime 依賴管理大量改善：
- 安裝至插件自己的 runtime 目錄
- 忽略 pnpm 的 `npm_execpath`，跳過 workspace-only specs
- 重複 Gateway 啟動時只記錄實際安裝的套件

### Session Maintenance 預設強制執行（2026.4.20）

Session maintenance 現在預設強制執行（#69404），防止 gateway OOM。這是重要的基礎設施改善，無需額外配置。
...
- Provider API key 輸入框新增 placeholder
- Wizard 支援可搜尋選取
...
- 修復 Discord、WhatsApp、Slack、Telegram 和 provider plugins 在 packaged 版本中可能缺失依賴的問題

```bash
# 執行 doctor 修復 bundled plugin 依賴
openclaw doctor --fix
```

### npm node-domexception 別名修復

修復 npm install 顯示 `node-domexception` 棄用警告的問題，安裝更乾淨。
...
- 首次 WhatsApp setup **不把 Baileys runtime dep 卡在 setup entry show 之前**（#70932）；登入前可 stage runtime deps

### 模型預設（2026.4.24）

- **DeepSeek**：**V4 Flash** 為新任 **onboarding** 預設；目錄並含 **V4 Pro**。
...
- **`openclaw migrate`**：自 **其他助手／IDE**（Claude Code／Desktop、Hermes 等）匯入設定與資源；支援 **dry-run／JSON／備份**（詳見英文 migrate 文件）。
