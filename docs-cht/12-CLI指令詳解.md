...
## CLI 改善（2026.4.13–2026.4.14）

### 新模型支援

| 指令/選項 | 變更 |
|-----------|------|
| `openclaw models set openai-codex/gpt-5.4-pro` | 新增 gpt-5.4-pro 前向相容（2026.4.14） |
| `openclaw models set openai-codex/gpt-5.4-codex` | 自動映射到 gpt-5.4（2026.4.14） |
| `openclaw models status` | 顯示 Gemini 修復後的正確狀態 |

### 效能改善

自 2026.4.14 起，`openclaw agent`、`openclaw configure`、`openclaw onboard` 等指令冷啟動速度顯著改善，因為大量重量級 runtime 模組改為懶載入。

### 安全改善

- **docker --repair**：`openclaw doctor --repair` 不再將 dotenv secrets 重新嵌入 systemd units（#66249）
...
| 指令 | 說明 |
|------|------|
| `openclaw exec-policy show` | 顯示當前 exec policy 設定 |
| `openclaw exec-policy preset <name>` | 套用 exec policy preset |
| `openclaw exec-policy set <key> <value>` | 設定 exec policy 值 |

### 更新指令改善

`openclaw update` 現在（#66959）：
...
成功的 CLI-backed turns（如 google-gemini-cli）持久化至 session transcript，在 Control UI 中可見。

### 全局安裝修復

修復全局安裝 CLI 在 onboarding 期間頻道選擇崩潰的問題（#66736）。

## CLI 改善（2026.4.18–2026.4.20）

### Configure 指令改善（2026.4.18）

- **頻道選擇前置**：`openclaw configure` 先顯示頻道選擇器，再探測狀態
- **Remove 模式**：允許 remove 模式直接從 config 刪除已配置的頻道 block

### QA Suite 失敗行為（2026.4.20）

```bash
# 場景失敗時預設返回非零退出碼（2026.4.20 新行為）
openclaw qa suite

# 允許失敗，用於 artifact-only 執行
...
```bash
# Setup hatch TUI 在新 process 重啟，安全傳遞 gateway target 和 auth source
openclaw setup
```

### CLI 交付路徑改善（2026.4.18）

- `openclaw agent --deliver` payloads 在發送前執行 reply-media path normalizer
- `MEDIA:./out/photo.png` 等相對路徑正確解析至 agent workspace

## CLI 改善（2026.4.22–2026.4.29）

### 聊天內建模型註冊（2026.4.22）

- 斜線 **`/models add <provider> <modelId>`**：註冊模型後可立刻使用，**無需重啟 Gateway**
...
- **`openclaw agent`**：`--agent` 與 `--session-id` 組合之查詢 **限於該 agent 之 session store**，避免錯認其他 agent 的 session（#70985）

### MCP 與單次執行（2026.4.24）

- **`openclaw agent`**／**`openclaw infer model run`** 等單次 Gateway／本機回合結束時：**回收 bundled MCP**，避免腳本連跑累積 **stdio MCP** 子程序（#71457）。

### Dev 工具（2026.4.24）

- **`openclaw dev reset`** 僅清除 **active profile** 相關狀態，降低誤傷其他環境。
...
*本文件基於 OpenClaw `2026.4.29` 版本撰寫。完整英文文件請參考 https://docs.openclaw.ai/cli*
