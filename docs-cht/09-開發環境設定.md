...
| `CLAWDBOT_DEBUG` | 除錯模式 |
| `CLAWDBOT_SKIP_GMAIL_WATCHER` | 跳過 Gmail 監聽器自動啟動 |
...
- GitHub Actions 使用共用的 `setup-node-env` action 統一環境設定
- Docker Release 的 `paths-ignore` 已擴展，跳過純文件變更
- 新增 `stale.yml` 工作流程，自動管理過期 Issues/PRs
- Labeler 工作流程已更新，支援更多擴充套件路徑
